...
| **Need to roll back to stock OEM firmware** | See [Going back to stock](#going-back-to-stock) below. HA and physical-button restores use the verified CDN image; the Web Installer and the device's own `/restore-stock` page can instead upload any locally saved OEM application image directly over the LAN; UART remains the universal offline fallback. |
...
| **Local OEM restore** | Open `http://<host>/restore-stock` or use the local-file section of the Web Installer | Streams a user-supplied OEM IT-AF-SMT application `.bin` directly to the inactive slot. This explicit restore marks the foreign slot valid and preserves OEM NVS; it is intentionally separate from normal replacement-firmware OTA. |

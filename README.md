...
| KEY1 release at ≥5 s | ≥ 5 s | **FACTORY RESET** — wipes NVS (Wi-Fi creds, all settings), reboots into Improv-BLE for re-onboarding | All 3 LEDs panic-blink (10 Hz) |
...
> **Partition-layout requirement for paths 1 and 2.** The HTTP OTA writes to whichever slot ESP-IDF picks as the next OTA target (typically the inactive one). Both `ota_0` and `ota_1` are 1.875 MB at the OEM addresses (`0x20000` and `0x200000`) — anything you flash must be built for that layout. The OEM bins fit by definition. This project's own firmware fits because [`partitions.csv`](partitions.csv) mirrors the OEM layout. Anything else — stock ESPHome images with default partitioning, third-party builds, etc. — will either refuse to write (too large for the slot) or write and fail to boot. Worst case, ESP-IDF rollback boots you back into the still-good previous slot on power-cycle. If you want to flash an alternative firmware via this path, build it from this project (or any project that ships an equivalent `partitions.csv`).
